...
run_simulation() {
    ...
    plot "$output_file1" "${parameter} vs Throughput"
    plot "$output_file2" "${parameter} vs Ratio"
}
...
run_simulation "Packet" "nPackets" "${numPackets_array[@]}"
run_simulation "Coverage Area" "coverageArea" "${coverage_array[@]}"
